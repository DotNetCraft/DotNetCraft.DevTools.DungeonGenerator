...
namespace DotNetCraft.DevTools.DungeonGenerator.Tests.Graphs
{
    ...
    {
        [TestMethod]
        public void SompleMinimumSpanningTreeBuildTest()
        {
            var graph = new Graph();
            ...
            var tree = graphAlgorithms.Kruskals_MST(graph, 1);
            Assert.AreEqual(3, tree.Vertices.Count);

            var edges = tree.Edges;
            Assert.AreEqual(1, edges[0].Vertex1);
            Assert.AreEqual(1, edges[1].Vertex1);

            Assert.AreEqual(3, edges[0].Vertex2);
        ...
        public void MinimumSpanningTreeBuildTest()
        {
            ...
            Assert.AreEqual(4, edges[1].Vertex1);
            Assert.AreEqual(4, edges[2].Vertex1);
            Assert.AreEqual(1, edges[3].Vertex1);
            Assert.AreEqual(5, edges[4].Vertex1);
            Assert.AreEqual(7, edges[5].Vertex1);
            Assert.AreEqual(2, edges[6].Vertex1);

            Assert.AreEqual(1, edges[0].Vertex2);
            Assert.AreEqual(5, edges[1].Vertex2);
            Assert.AreEqual(7, edges[2].Vertex2);
            Assert.AreEqual(2, edges[3].Vertex2);
            Assert.AreEqual(6, edges[4].Vertex2);
            Assert.AreEqual(8, edges[5].Vertex2);
            Assert.AreEqual(3, edges[6].Vertex2);
